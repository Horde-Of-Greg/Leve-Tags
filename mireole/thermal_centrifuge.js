...
if(user.username == "mireole" || (based.includes(user.id) && !based.includes(msg.author.id))) {
  msg.reply("Thermal centrifuging backfired due to a lack of skill, thermal centrifuged <@" + msg.author.id + "> instead");
}
else {
  msg.reply("Successfully thermal centrifuged <@" + user.id + ">");
}
